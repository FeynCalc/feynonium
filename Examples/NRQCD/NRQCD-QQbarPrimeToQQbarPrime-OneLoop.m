...
(* ::Subsection:: *)
(*Load FeynCalc, FeynArts and FeynHelpers and FeynOnium*)
...
amp=FCFAConvert[CreateFeynAmp[diags,PreFactor->-1],
...
(* ::Text:: *)
(*Using "naive" scheme for Pauli matrices in D-1 dimensions, i.e. the *)
...
FMSpinorChainExplicit2[#,FMSpinorNormalization->"nonrelativistic"]&//
	...
	CartesianPair[CartesianMomentum[ST2,D-1],x]}]&//Contract//
	Collect2[#,{ST1,ST2}]&;]//First
...
AbsoluteTiming[amp5=Series[amp4//FCReplaceD[#,D->4-2EpsilonIR]&//
DotSimplify//FCShowEpsilon,{EpsilonIR,0,0}]//Normal//
FCHideEpsilon;]//First
...
AbsoluteTiming[finitePartEval=(finitePart/.D->4)//SUNSimplify//
Expand2[#,SUNTF]&//ReplaceAll[#,ruleColor]&;]//First
...
matchingCoeff["d_ss"]=getMatchingCoeff[spinSingletColorSingletFin,
{Log[m1]->1/2Log[(m1/ScaleMu)^2]+
Log[ScaleMu],Log[m2]->1/2Log[(m2/ScaleMu)^2]+Log[ScaleMu]}]


matchingCoeff["d_sv"]=getMatchingCoeff[spinTripletColorSingletFin,
{Log[m1]->1/2Log[(m1/m2)^2]+Log[m2]}]


matchingCoeff["d_vs"]=getMatchingCoeff[spinSingletColorOctetFin,
{Log[m1]->1/2Log[(m1/ScaleMu)^2]+Log[ScaleMu],
Log[m2]->1/2Log[(m2/ScaleMu)^2]+Log[ScaleMu]}]//Collect2[#,CA,CF]&


matchingCoeff["d_vv"]=getMatchingCoeff[spinTripletColorOctetFin,
{Log[m1]->1/2Log[(m1/ScaleMu)^2]+
Log[ScaleMu],Log[m2]->1/2Log[(m2/ScaleMu)^2]+Log[ScaleMu]}]//
Collect2[#,CA,CF]&
...
matchingCoeffPinedaSoto["d_ss"]=- CF(CA/2-CF) SMP["alpha_s"]^2/
(m1^2-m2^2)(m1^2(Log[m2^2/ScaleMu^2]+1/3)-m2^2(Log[m1^2/ScaleMu^2]+1/3));
matchingCoeffPinedaSoto["d_sv"]= CF(CA/2-CF) SMP["alpha_s"]^2/
(m1^2-m2^2)m1 m2 Log[m1^2/m2^2];
matchingCoeffPinedaSoto["d_vs"]= -2CF SMP["alpha_s"]^2/
...
matchingCoeffPinedaSoto["d_vv"]= 2CF SMP["alpha_s"]^2/
...
Print["Check with Eq. 3.1-3.4 in arXiv:hep-ph/9802365: ",
			If[Simplify[PowerExpand[{matchingCoeff["d_ss"]-
			matchingCoeffPinedaSoto["d_ss"],
matchingCoeff["d_sv"]-matchingCoeffPinedaSoto["d_sv"],
matchingCoeff["d_vs"]-matchingCoeffPinedaSoto["d_vs"],
matchingCoeff["d_vv"]-matchingCoeffPinedaSoto["d_vv"]}]]==={0,0,0,0},
"CORRECT.", "!!! WRONG !!!"]];
