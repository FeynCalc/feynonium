...
NRQCD for the exlusive electromagnetic decay of a J/Psi into three photons \
using NRQCD factorization at the level of total decay rates."];
];
...
diags=InsertFields[CreateTopologies[0,2->3],{F[3,{2}],-F[3,{2}]}->{V[1],V[1],V[1]},
InsertionLevel->{Classes},Model->"SMQCD"];
Paint[diags,ColumnsXRows->{3,2},Numbering->None,SheetHeader->False,ImageSize->{756,512}];


ampsRaw=((3/2)^3 SMP["e_Q"]^3*FCFAConvert[CreateFeynAmp[diags,Truncated->False,PreFactor->-1],
IncomingMomenta->{p1,p2},OutgoingMomenta->{k1,k2,k3},
UndoChiralSplittings->True,List->False,TransversePolarizationVectors->{k1,k2,k3},
ChangeDimension->4,List->True,SMP->True,DropSumOver->True,
...
amps=(MomentumExpand[ampsRaw]/.Momentum[k3]->Momentum[p1+p2-k1-k2])//MomentumExpand
...
AbsoluteTiming[ampST=Collect2[SelectNotFree2[amps3,PauliSigma],PauliSigma,Polarization,FCFactorOut->QCharge^3]//
ReplaceAll[#,{PauliEta[-I].PauliSigma[x_].PauliXi[I]:>CartesianPair[CartesianMomentum[ST],x],
PauliXi[-I].PauliSigma[x_].PauliEta[I]:>CartesianPair[CartesianMomentum[STcc],x]}]&;]//First
...
AbsoluteTiming[ampSTSquared1=(ComplexConjugate[ampST/.ST->STcc] ampST);]//First
...
AbsoluteTiming[ampSTSquared2=ampSTSquared1//DoPolarizationSums[#,k1,aux]&//DoPolarizationSums[#,k2,aux]&//
...
AbsoluteTiming[ampSTSquared3=ampSTSquared2//Uncontract[#,k3hat,CartesianPair->All]&//
ReplaceRepeated[#,{CartesianMomentum[k3hat]:>-k1abs/k3abs CartesianMomentum[k1hat]-
k2abs/k3abs CartesianMomentum[k2hat],k3abs->2Eq-k1abs-k2abs}]&//ExpandScalarProduct//
Contract//Collect2[#,k1hat,k2hat]&;]//First
...
AbsoluteTiming[ampSTSquared4=FMCartesianTensorDecomposition[ampSTSquared3,{k1hat,k2hat},0]//Simplify;]//First
...
QCDDecayRate=ReplaceRepeated[QMass^2/(192 Pi^3)ampSTSquared4,{SMP["e"]->2 Sqrt[Pi SMP["alpha_fs"]],
(x1^4+2 x1^3 (-2+x2)+(-1+x2)^2 (2-2 x2+x2^2)+x1^2 (7-9 x2+3 x2^2)+x1 (-6+13 x2-9 x2^2+2 x2^3))/(x1^2 x2^2 (-2+x1+x2)^2)->1/2(Pi^2-9)}]
...
(*tmp=Integrate[(x1^4+2 x1^3 (-2+x2)+(-1+x2)^2 (2-2 x2+x2^2)+x1^2 (7-9 x2+3 x2^2)+x1 (-6+13 x2-9 x2^2+2 x2^3))/(x1^2 x2^2 (-2+x1+x2)^2),{x2,1-x1,1}]
...
matchingCoeff[ImFem3S1]=Solve[QCDDecayRate==NRQCDDecayRate,{ImFem3S1}]//Flatten
...
Print["Check with Eq.1 in arXiv:1210.6337: ",
			If[FCI[resLit]-NRQCDDecayRateFinal===0, "CORRECT.", "!!! WRONG !!!"]];
